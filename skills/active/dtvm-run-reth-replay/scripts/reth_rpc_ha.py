...
OFFLINE_NETWORK_ISOLATION_SCHEMA = "libseccomp-deny-network-syscalls.v1"
...
def load_config(path: Path) -> Config:
    ...
    policy = Policy(
        ...
        max_attempts=integer("maxAttempts", 4, 2, 32),
    ...
    def snapshot(self) -> dict[str, Any]:
        with self.lock:
            ...
            return {
                ...
                "quorumUnavailable": self.quorum_unavailable,
...
class RpcClient:
    ...
    def quorum_call(
        ...
        *,
        ...
        if len(winners) != 1:
            conflicting_quorum = bool(
                expected_key is not None
                and any(
                    key != expected_key
                    and len(values) >= self.config.policy.canonical_quorum
                    for key, values in groups.items()
                )
            )
            category = (
                "canonical_quorum_disagreement"
                if len(groups) > 1 or conflicting_quorum
                else "canonical_quorum_unavailable"
            )
            if category == "canonical_quorum_disagreement":
                self.metrics.increment("quorum_disagreements")
            else:
                self.metrics.increment("quorum_unavailable")
...
def configured_seccomp_library() -> Any:
    try:
        ...
        raise ConfigError("offline_network_isolation_unavailable") from error
    return library


def resolve_offline_network_syscalls(
    resolver: Callable[[bytes], int],
) -> list[tuple[str, int]]:
    resolved: list[tuple[str, int]] = []
    for name in OFFLINE_REQUIRED_NETWORK_SYSCALLS:
        syscall = resolver(name.encode("ascii"))
        if syscall < 0:
            raise ConfigError("offline_network_isolation_unavailable")
        resolved.append((name, syscall))
    for name in OFFLINE_OPTIONAL_NETWORK_SYSCALLS:
        syscall = resolver(name.encode("ascii"))
        if syscall >= 0:
            resolved.append((name, syscall))
    return resolved
...
def install_offline_network_filter() -> None:
    """Deny socket syscalls in this process and every descendant."""
    library = configured_seccomp_library()
    resolved = resolve_offline_network_syscalls(
        library.seccomp_syscall_resolve_name
    )
    context = library.seccomp_init(ctypes.c_uint32(0x7FFF0000))
    if not context:
        raise ConfigError("offline_network_isolation_unavailable")
    try:
        deny_with_eperm = ctypes.c_uint32(0x00050000 | errno.EPERM)
        for _name, syscall in resolved:
            if (
...
def offline_exec(arguments: list[str]) -> int:
    if not arguments:
        return 125
    try:
        install_offline_network_filter()
        os.execvpe(arguments[0], arguments, dict(os.environ))
    except (ConfigError, OSError):
        return 125
    return 125


def offline_network_isolation_identity() -> dict[str, Any]:
    runner = lexical_absolute(Path(__file__))
    require_regular_no_symlink(
        runner,
        "offline_network_isolation_runner_missing_or_symlinked",
    )
    library = configured_seccomp_library()
    resolved = resolve_offline_network_syscalls(
        library.seccomp_syscall_resolve_name
    ...
    return {
        "schema": OFFLINE_NETWORK_ISOLATION_SCHEMA,
        "runnerRealpath": str(runner),
        "runnerSha256": sha256_file(runner),
        "deniedSyscalls": [name for name, _syscall in resolved],
    }
...
def run_offline_subprocess(
    ...
    launcher = [
        sys.executable,
        str(lexical_absolute(Path(__file__))),
        "__offline_exec",
    ]
    probe_source = (
        "import errno,socket,sys\n"
        "if sum(range(101)) != 5050:\n"
        "    sys.exit(4)\n"
    ...
    try:
        probe = subprocess.run(
            [*launcher, sys.executable, "-c", probe_source],
            env=environment,
...
def run_replay_locked(
    ...
    offline_isolation = offline_network_isolation_identity()
    ...
    if state.get("strictReplayCompleted") is True:
        if (
            ...
            or state.get("offlineReplayIsolation") != offline_isolation
        ):
    ...
    try:
        process = run_offline_subprocess(
            command,
            environment,
            args.replay_timeout,
        )
        ...
        update_state(
            ...
            state,
            status="in_progress",
            lastFailureCategory="strict_replay_timeout",
        )
        raise RpcFailure("strict_replay_timeout") from None
    ...
    update_state(
        ...
        offlineReplayIsolation=offline_isolation,
...
def seal_inputs(
    ...
    inputs = [
        ...
        {
            "role": "offline_replay_launcher",
            "path": state["offlineReplayIsolation"]["runnerRealpath"],
            "sha256": state["offlineReplayIsolation"]["runnerSha256"],
        },
        {
            "role": "rpc_metrics",
...
def run_seal_locked(config: Config, state_dir: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ConfigError("approved_replayer_identity_changed")
    if (
        state.get("offlineReplayIsolation")
        != offline_network_isolation_identity()
    ):
        raise ConfigError("offline_network_isolation_identity_changed")
    output = Path(state["output"])
...
def main() -> int:
    if len(sys.argv) > 1 and sys.argv[1] == "__offline_exec":
        return offline_exec(sys.argv[2:])
    parser = build_parser()
