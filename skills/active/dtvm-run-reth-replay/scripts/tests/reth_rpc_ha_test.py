...
class RethRpcHaTests(unittest.TestCase):
    ...
    def test_partial_vote_is_unavailable_not_disagreement(self) -> None:
        def unavailable(
            request: dict[str, Any],
        ) -> tuple[int, Any, dict[str, str], float]:
            if request["method"] == "eth_getBlockByNumber":
        ...
        with fake_cluster([None, unavailable]) as servers:
            path = write_config(self.root, attempts=2, quorum=2)
            value = json.loads(path.read_text())
            value["endpoints"] = value["endpoints"][:2]
            path.write_text(json.dumps(value))
        ...
        self.assertEqual(metrics.quorum_unavailable, 1)
        self.assertEqual(metrics.quorum_disagreements, 0)
    ...
    def test_config_requires_mainnet_identity_and_unique_url_env(self) -> None:
        ...
        path.write_text(json.dumps(value))
        with self.assertRaisesRegex(
            HA.ConfigError, "invalid_policy_maxAttempts"
        ):
            HA.load_config(path)
    ...
    def test_offline_launcher_denies_socket_syscalls(self) -> None:
        config = HA.load_config(write_config(self.root))
        with mock.patch.dict(
            os.environ,
            {
                "TEST_PRIMARY_URL": f"https://primary.invalid/{SECRET}",
                "TEST_STANDBY_URL": f"https://standby.invalid/{SECRET}",
                "TEST_AUX_URL": f"https://aux.invalid/{SECRET}",
                "HTTP_PROXY": f"http://proxy.invalid/{SECRET}",
                "WEB3_PROVIDER_URL": f"https://web3.invalid/{SECRET}",
            },
            clear=False,
        ):
            environment = HA.offline_environment(config)
        for name in (
            "TEST_PRIMARY_URL",
            "TEST_STANDBY_URL",
            "TEST_AUX_URL",
        ...
        probe = HA.run_offline_subprocess(
            [
                sys.executable,
                "-c",
                (
        ...
        identity = HA.offline_network_isolation_identity()
        self.assertEqual(
            identity["schema"],
            HA.OFFLINE_NETWORK_ISOLATION_SCHEMA,
        )
        self.assertIn("socket", identity["deniedSyscalls"])
        self.assertIn("io_uring_setup", identity["deniedSyscalls"])
        self.assertIn("io_uring_enter", identity["deniedSyscalls"])
        self.assertIn("io_uring_register", identity["deniedSyscalls"])
        self.assertRegex(identity["runnerSha256"], r"^[0-9a-f]{64}$")

        for missing in HA.OFFLINE_REQUIRED_NETWORK_SYSCALLS:
            with self.subTest(unresolved=missing):
                resolver = mock.Mock(
                    side_effect=lambda name, missing=missing: (
                        -1 if name.decode("ascii") == missing else 1
                    )
                )
                with self.assertRaises(HA.ConfigError) as unresolved:
                    HA.resolve_offline_network_syscalls(resolver)
                self.assertEqual(
                    str(unresolved.exception),
                    "offline_network_isolation_unavailable",
                )
                self.assertNotIn(missing, str(unresolved.exception))
        optional_resolver = mock.Mock(
            side_effect=lambda name: (
                -1
                if name.decode("ascii")
                in HA.OFFLINE_OPTIONAL_NETWORK_SYSCALLS
                else 1
            )
        )
        resolved = HA.resolve_offline_network_syscalls(optional_resolver)
        self.assertNotIn(
            "socketcall",
            {name for name, _syscall in resolved},
        )
        ...
        with mock.patch.object(
            HA.subprocess,
            "run",
            side_effect=[successful_probe, OSError(SECRET)],
        ) as run:
            with self.assertRaises(HA.ConfigError) as raised:
                HA.run_offline_subprocess(
        ...
        for call in run.call_args_list:
            ...
            self.assertNotIn("close_fds", call.kwargs)
        probe_source = run.call_args_list[0].args[0][-1]
        self.assertIn("sum(range(101))", probe_source)
        self.assertIn("socket.AF_INET", probe_source)
        self.assertIn("socket.AF_INET6", probe_source)
...
"""
            ...
            if validator:
                changed_isolation = json.loads(original_replayed_state)
                changed_isolation["offlineReplayIsolation"][
                    "deniedSyscalls"
                ].remove("io_uring_setup")
                state_path.write_text(json.dumps(changed_isolation))
                validation = validate_state("failure")
                self.assertNotEqual(validation.returncode, 0)
