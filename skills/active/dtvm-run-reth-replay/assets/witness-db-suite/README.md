...
The 30 files are an adapter slice, not a standalone Cargo dependency closure.
Rust build/test additionally requires the frozen experiment parent adapter,
`reth-transaction`, Reth source tree, and pinned toolchain at the relative
paths declared by `Cargo.toml`. The DTVMDotfiles script validation hook does
not substitute for that external Rust gate.
...
## Historical experiment evidence is not a DTVMDotfiles validation result

The counts below describe the source experiment from which this slice was
sealed. Do not cite them as a current result unless the referenced evidence,
external dependency layout, toolchain, DTVM library, and Rust gates have been
independently revalidated. The DTVMDotfiles hook runs only deterministic
Python/shell checks.
...
The HA layer adds 21 hermetic Python scenarios for primary failure, standby
...
workflow serialization, idempotent evidence sealing, partial-vote
unavailability classification, libseccomp socket/io_uring replay isolation,
