...
| Matching votes below quorum | Report `canonical_quorum_unavailable` | Stop until enough eligible sources return |
...
source for its 30-file adapter slice. Restore a working slice from any clean
checkout before operating it:
...
This slice is not a standalone Cargo dependency closure: its path dependencies
also require the frozen experiment parent adapter, `reth-transaction`, and Reth
source tree. Run Rust build/test only in that complete frozen layout with the
pinned toolchain. The DTVMDotfiles validation hook covers deterministic
Python/shell behavior and must not be reported as a passing Rust gate.
...
Start or resume the latest finalized 16-block capture without relying on an
unversioned experiment wrapper:

```bash
PYTHONDONTWRITEBYTECODE=1 python3 reth_rpc_ha.py \
  --config /private/path/reth-rpc-ha.json capture \
  --count 16 \
  --output /private/path/finalized-16-corpus \
...
environment, then runs a fail-closed libseccomp launcher that denies socket,
io_uring setup/enter/register, and architecture-specific `socketcall` when
exposed. Every current-architecture required syscall name must resolve before
the filter loads. Its probe must run pure computation while AF_INET and AF_INET6
socket creation both fail with EPERM.
The workflow verifies the expected SHA-256 of both `verify-corpus.sh` and the
DTVM library, revalidates the capture-time approved replayer manifest and
binary bytes, and requires the result to prove every per-block differential
and commitment check. The result must repeat the capture manifest SHA-256 and
block count, bind every ordered block number/hash/bundle path/bundle SHA-256,
and repeat the approved replayer realpath and SHA-256. The replayer manifest
is mandatory for HA capture; its approved binary and the offline launcher
identity are included in the final evidence seal. Replay must also state that
network timing is excluded from the performance conclusion. Run `seal` only
after replay:
